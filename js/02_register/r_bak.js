...
//	提交表单按钮
    $("#submit").click(()=>{
        //验证必填项,是否已经完成验证
        var data={};
        var must = [$("#uname"),$("#upwd"),$("#upwd2"),$("#email")];
        for(var m of must){
            if(m.nextAll(".vali_msg").html()=="×"||m.nextAll(".vali_msg").html()==""){
                m.blur();
                m.focus();
                return;
            }
        ...
        //验证选填项,是否已经完成验证
        var not_must = [$("#user_name"),$("#phone"),$("user_id")];
        for(var n of not_must){
            if(n.nextAll(".vali_msg").html()=="×"){
                n.click();
                return;
            }
        }
